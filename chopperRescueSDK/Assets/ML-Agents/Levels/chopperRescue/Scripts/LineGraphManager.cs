...
public float index = 10f;
	...
	void Start(){ //Update(){ 
		...
		// adding random data
		float index = Random.Range(20,120);
       // int[] index = { 5, 56, 16, 45, 30, 22, 17, 15, 13, 17, 25, 37, 40, 36, 33, 45 };
		for(float i = 0; i < index; i++){		
		//for(int i = 0; i < index.Length; i++){
			GraphData gd = new GraphData();
			gd.marbles = Random.Range(10,47); //index [i];
			graphDataPlayer1.Add(gd);
			GraphData gd2 = new GraphData();
			gd2.marbles =  Random.Range(10,47); // index [i]/2;
		...
		/*public void AdjustIndex(int newIndex) {
		index = newIndex;
	}*/

	public void AdjustIndex(float newIndex) {
		index = newIndex;
	}
